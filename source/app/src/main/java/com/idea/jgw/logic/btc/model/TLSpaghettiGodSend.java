...
public class TLSpaghettiGodSend {
    ...
    public TLSendFormData.TLSelectObjectType getSelectedObjectType() {
        ...
        return TLSendFormData.TLSelectObjectType.Unknown;
    }
    ...
    private void getAndSetUnspentOutputs(final TLSpaghettiGodSend self, final TLCallback callback) {
        if (sendFromAccounts != null && sendFromAccounts.size() != 0) {
            TLAccountObject accountObject = sendFromAccounts.get(0);
            ...
            final List<String> addresses = new ArrayList<>(sendFromAddresses.size());
            TLImportedAddress importedAddress = sendFromAddresses.get(0);
    ...
    public CreatedTransactionObject createSignedSerializedTransactionHex(List<Pair<String, TLCoin>> toAddressesAndAmounts,
        ...
        Collections.sort(inputsData, new Comparator<InputDataObject>() {
            @Override
            public int compare(InputDataObject a, InputDataObject b) {
        ...
        Collections.sort(outputsData, new Comparator<OutputDataObject>() {
            @Override
            public int compare(OutputDataObject a, OutputDataObject b) {
                ...
                if (a.amount < b.amount) {
                    ...
                    String firstScript = TLBitcoinjWrapper.getStandardPubKeyHashScriptFromAddress(a.to_address, appWallet.walletConfig.isTestnet);
                    String secondScript = TLBitcoinjWrapper.getStandardPubKeyHashScriptFromAddress(b.to_address, appWallet.walletConfig.isTestnet);
