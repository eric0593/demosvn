package com.idea.jgw.logic.btc.network;
...
public class TLPushTxAPI {
    ...
    public void sendTx(final String txHex, final String txHash,final String toAddress, final TLCallback callback) {
        Log.d(TAG, "sendTx");
    ...
    void lookupTx(final String toAddress, final String txid, final TLCallback callback) {
        final Handler handler = new Handler(Looper.getMainLooper()) {
            @Override
            public void handleMessage(Message msg) {
                JSONObject jsonData = (JSONObject) msg.obj;
