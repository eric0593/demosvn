...
import com.idea.jgw.logic.btc.utils.TLWalletUtils.TLAccountType;
...
public class TLAccountObject {
    ...
    private Map<String, TLWalletUtils.TLAccountTxType> txidToAccountAmountTypeDict = new HashMap<String, TLWalletUtils.TLAccountTxType>();
    private List<String> receivingAddressesArray = new ArrayList<String>();
    private Set<String> processedTxDict = new HashSet<String>();
    private TLWalletUtils.TLAccountType accountType;
    ...
    public TLOperationsManager.TLDownloadState downloadState = TLOperationsManager.TLDownloadState.NotDownloading;
    ...
    TLAccountObject(TLAppDelegate appDelegate, TLWallet appWallet, JSONObject dict, TLWalletUtils.TLAccountType accountType) {
        ...
        txidToAccountAmountTypeDict = new HashMap<String, TLWalletUtils.TLAccountTxType>();
        ...
        if (accountType == TLWalletUtils.TLAccountType.HDWallet) {
            positionInWalletArray = getAccountIdxNumber();
        } else if (accountType == TLWalletUtils.TLAccountType.ColdWallet) {
        ...
        try {
            if (TLWalletUtils.ALLOW_MANUAL_SCAN_FOR_STEALTH_PAYMENT() && accountType != TLWalletUtils.TLAccountType.ImportedWatch && accountType != TLWalletUtils.TLAccountType.ColdWallet) {
    ...
    public boolean isWatchOnly() {
        return accountType == TLWalletUtils.TLAccountType.ImportedWatch;
    }

    public boolean isColdWalletAccount() {
        return accountType == TLWalletUtils.TLAccountType.ColdWallet;
    }

    public boolean hasSetExtendedPrivateKeyInMemory() {
        assert (accountType == TLWalletUtils.TLAccountType.ImportedWatch);
    ...
    public String getMainPrivateKey(String address) {
        try {
            ...
            if (accountType == TLWalletUtils.TLAccountType.ImportedWatch) {
    ...
    TLWalletUtils.TLAccountTxType getAccountAmountChangeTypeForTx(String txHash) {
    ...
    private TLCoin processTx(TLTxObject txObject, boolean shouldCheckToAddressesNTxsCount, boolean shouldUpdateAccountBalance) {
        ...
        if (currentTxSubtract > currentTxAdd) {
            ...
            txidToAccountAmountTypeDict.put(txObject.getHash(), TLWalletUtils.TLAccountTxType.Send);
            return null;
        } else if (currentTxSubtract < currentTxAdd) {
            TLCoin amountChangeToAccountFromTx = new TLCoin(currentTxAdd - currentTxSubtract);
            txidToAccountAmountDict.put(txObject.getHash(), amountChangeToAccountFromTx);
            txidToAccountAmountTypeDict.put(txObject.getHash(), TLWalletUtils.TLAccountTxType.Receive);
            return amountChangeToAccountFromTx;
        } else {
            TLCoin amountChangeToAccountFromTx = TLCoin.zero();
            txidToAccountAmountDict.put(txObject.getHash(), amountChangeToAccountFromTx);
            txidToAccountAmountTypeDict.put(txObject.getHash(), TLWalletUtils.TLAccountTxType.MoveBetweenAccount);
            return null;
        }
    }
...
//                Log.d("TLAccountObject", "getNewMainAddress HDIdx: "+ i +" address: %@" + address);
    ...
    public void recoverAccount(final boolean shouldResetAccountBalance, final boolean recoverStealthPayments, final TLCallback callback) {
    ...
    private void getUnspentOutputs(final TLAccountObject self, final TLCallback callback) {
        final List<String> activeAddresses = new ArrayList<String>(getActiveMainAddresses());
        ...
        final Handler handler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message msg) {
                ...
                try {
                    ...
                    Collections.sort(sortedUnspentOutputs,
                    ...
                    Collections.sort(sortedStealthPaymentUnspentOutputs, new Comparator<JSONObject>() {
                        @Override
                        public int compare(JSONObject a, JSONObject b) {
                            long first;
                            try {
                    ...
                    self.stealthPaymentUnspentOutputs = new

                            JSONArray();
                    for (
                            int i = 0; i < sortedStealthPaymentUnspentOutputs.size(); i++)
    ...
    void fetchNewStealthPaymentsAsync(final boolean isRestoringAccount) {
        final Handler handler = new Handler(Looper.getMainLooper()) {
            @Override
            public void handleMessage(Message msg) {
    ...
    void fetchNewStealthPayments(boolean isRestoringAccount) {
        ...
        while (true) {
            TLStealthWallet.GetAndStoreStealthPaymentsResults ret = this.stealthWallet.getAndStoreStealthPayments(offset);
            if (ret == null) {
                break;
            }
    ...
    private boolean getAccountData(List<String> addresses, boolean shouldResetAccountBalance, TLAccountObject self) {
        try {
            ...
            try {
                ...
                self.downloadState = TLOperationsManager.TLDownloadState.Downloaded;
    ...
    void addStealthAddressPaymentKey(String privateKey, String address, String txid, long txTime, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus) {
        if (accountType == TLAccountType.HDWallet) {
            int accountIdx = this.getAccountIdxNumber();
    ...
    void setStealthPaymentStatus(String txid, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        if (accountType == TLAccountType.HDWallet) {
            int accountIdx = this.getAccountIdxNumber();
    ...
    public void getAccountDataO(boolean fetchDataAgain, TLCallback callback) {
        if (this.downloadState == TLOperationsManager.TLDownloadState.QueuedForDownloading || this.downloadState == TLOperationsManager.TLDownloadState.Downloading
                || (!fetchDataAgain && this.downloadState == TLOperationsManager.TLDownloadState.Downloaded)) {
            if (callback != null) {
        ...
        if (needsRecovering()) {
            this.downloadState = TLOperationsManager.TLDownloadState.Failed;
            if (callback != null) {
                callback.onFail(-1000, "Local failed");
            }
            return;
    ...
    private void getAccountDataO(final List<String> addresses, final boolean shouldResetAccountBalance, final TLCallback callback) {
        final Handler handler = new Handler(Looper.getMainLooper()) {
            @Override
            public void handleMessage(Message msg) {
                JSONObject jsonData = (JSONObject) msg.obj;
    ...
    private void onAfterFetchAccountData(JSONObject jsonData, boolean shouldResetAccountBalance, TLCallback callback) {
        ...
        try {
            ...
            this.downloadState = TLOperationsManager.TLDownloadState.Downloaded;
