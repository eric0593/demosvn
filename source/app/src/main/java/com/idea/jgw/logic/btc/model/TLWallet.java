...
import com.idea.jgw.logic.btc.utils.TLUtils;
...
public class TLWallet {
    ...
    private TLAccountObject getAccountObjectForIdx(int accountIdx) {
        try {
            JSONArray accountsArray = getAccountsArray();
            JSONObject accountDict = accountsArray.getJSONObject(accountIdx);
            return new TLAccountObject(this.appDelegate, this, accountDict, TLWalletUtils.TLAccountType.HDWallet);
        } catch (JSONException e) {
            return null;
        }
    }
    ...
    private void addStealthAddressPaymentKey(JSONObject accountDict, String privateKey,
                                             String address, String txid, long txTime, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus) {
        try {
    ...
    void setStealthPaymentStatus(JSONObject accountDict, String txid,
                                 TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        try {
            JSONArray stealthAddressArray = accountDict.getJSONArray(TLWalletJSONKeys.WALLET_PAYLOAD_KEY_STEALTH_ADDRESSES);
            JSONArray paymentsArray = stealthAddressArray.getJSONObject(0).getJSONArray(TLWalletJSONKeys.WALLET_PAYLOAD_KEY_PAYMENTS);
            for (int i = 0; i < paymentsArray.length(); i++) {
    ...
    void setStealthPaymentStatusHDWallet(int accountIdx, String txid, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        JSONObject accountDict = getAccountDict(accountIdx);
        setStealthPaymentStatus(accountDict,  txid, stealthPaymentStatus, lastCheckTime);
    }

    void setStealthPaymentStatusColdWalletAccount(int idx, String txid, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        JSONObject accountDict = getColdWalletAccountAtIndex(idx);
        setStealthPaymentStatus(accountDict,  txid, stealthPaymentStatus, lastCheckTime);
    }

    void setStealthPaymentStatusImportedAccount(int idx, String txid, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        JSONObject accountDict = getImportedAccountAtIndex(idx);
        setStealthPaymentStatus(accountDict,  txid, stealthPaymentStatus, lastCheckTime);
    }

    void setStealthPaymentStatusImportedWatchAccount(int idx, String txid, TLWalletJSONKeys.TLStealthPaymentStatus stealthPaymentStatus, long lastCheckTime) {
        JSONObject accountDict = getImportedWatchOnlyAccountAtIndex(idx);
        setStealthPaymentStatus(accountDict,  txid, stealthPaymentStatus, lastCheckTime);
    }
    ...
    void removeOldStealthPayments(JSONObject accountDict) {
        try {
            ...
            while (stealthAddressPaymentsArray.length() > TLStealthExplorerAPI.STEALTH_PAYMENTS_FETCH_COUNT) {
                ...
                if (stealthAddressPaymentDict.getInt(TLWalletJSONKeys.WALLET_PAYLOAD_KEY_STATUS) == TLWalletJSONKeys.TLStealthPaymentStatus.Spent.getValue()) {
